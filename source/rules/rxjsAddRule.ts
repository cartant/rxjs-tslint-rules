...
class Walker extends UsedWalker {
  ...
  protected onSourceFileEnd(): void {
    ...
    if (options && options.file) {
      ...
      if (
        this.normalizeFile(this.sourceFilePath) ===
        ...
        if (!options.allowUnused) {
          ...
          for (let i = 0, length = sourceFiles.length; i < length; ++i) {
            ...
            if (!sourceFile["isDeclarationFile"]) {
              const sourceFileWalker = new UsedWalker(
                sourceFile,
                {
                  disabledIntervals: [],
                  ruleArguments: [],
  ...
  private findSourceFile(file: string): ts.SourceFile {
    ...
    if (fileDirCandidates.size === 0) {
      ...
      throw new Error(
        `Cannot find an import of '${file}' from any of these locations: ` +
          `[${Array.from(fileDirCandidates.values()).join(
            ", "
