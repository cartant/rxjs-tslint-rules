...
export class Rule extends Lint.Rules.TypedRule {
    ...
    private validateArgs(
        ...
        const allowNext = (options && options.hasOwnProperty("allowNext")) ? options.allowNext : true;
        ...
        if (args.length > 1) {
            ...
            if (tsutils.isFunctionExpression(arg) || tsutils.isArrowFunction(arg) || couldBeFunction(type)) {
                failures.push(new Lint.RuleFailure(
                    sourceFile,
                    arg.getStart(),
                    arg.getStart() + arg.getWidth(),
