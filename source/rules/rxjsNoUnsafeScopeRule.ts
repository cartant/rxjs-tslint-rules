...
class Walker extends ScopeWalker {
  ...
  private isUnsafe(node: ts.Node): ts.Node | undefined {
    ...
    const nodeDeclaration = findDeclaration(node, typeChecker);
    if (!nodeDeclaration) {
      return undefined;
    }
    if (isWithinClosure(nodeDeclaration, rootCallback)) {
      return undefined;
    }
    ...
    if (tsutils.isPropertyAccessExpression(node.parent)) {
      ...
      const rootDeclaration = findDeclaration(root, typeChecker);
      if (!rootDeclaration) {
        return undefined;
      }
      if (isWithinClosure(rootDeclaration, rootCallback)) {
        return undefined;
      }

      if (
        this.allowParameters &&
        isWithinParameterDeclaration(rootDeclaration)
      ) {
        return undefined;
      }
      return root;
    ...
    if (this.allowParameters && isWithinParameterDeclaration(nodeDeclaration)) {
      return undefined;
    }
    ...
    if (isConstDeclaration(nodeDeclaration)) {
      return undefined;
    }

    if (tsutils.isImportSpecifier(nodeDeclaration)) {
      return undefined;
    }

    if (tsutils.isNamespaceImport(nodeDeclaration)) {
      return undefined;
    }
    return node;
  }
